...
**Status (2026-09-09): ALL 4 GROUPS ARE DONE. All 92 Johnson solids
are now in the registry** (batch 12, the 8 Group 4 shapes J85-J92,
completes it). The tooling needed was overstated originally — see
Group 4's own corrected section: 6 of the 8 needed only
`numpy.roots()` on a published polynomial (already available, nothing
installed); the other 2 (J91, J92) were pure golden-ratio closed forms
needing no root-finding at all. Full record, including 2 real findings
that only surfaced during construction (J85 is achiral despite its
name; J90's own source describes its symmetry group imprecisely), is
in Group 4's section below and in `docs/build-plan.md`'s batch 12
postmortem.
...
**Confirmed and fixed (2026-09-09, batch 12): all 8.** The protocol
above worked exactly as designed for every shape, with two genuine
findings surfacing only during construction, not predicted in advance:

- **J85 (snub square antiprism) turned out to be ACHIRAL**, despite
  "snub" suggesting the chiral snub-solid family already in this
  registry (`SNUB_CUBE`, `SNUB_DODECAHEDRON`, J44-J48). Checked
  directly rather than assumed from the name: its own symmetry group
  (order 16, generated by a 90° z-rotation and a perpendicular 180°
  rotation — both proper rotations) was found to map its mirror image
  exactly back onto itself, point for point.
- **J90 (disphenocingulum)'s own Wikipedia source under-specifies its
  ...
  — but that only generates a Klein-four group (order 4, max orbit 4
  per point), which cannot reach the article's own stated 16 vertices
  from 3 points. Resolved by testing directly: the correct group is
  the full D2d (order 8), needing an S4 rotoreflection generator the
  prose omitted — confirmed by matching the article's own separately-
  stated vertex-configuration multiplicities (4/4/8) and by exact
  unit-edge verification, not by trusting the prose description alone.

All 8 passed cleanly: exact published V/E/F and face composition for
every shape, all edges unit length, correct outward winding, zero
non-square quads. Verified end to end against the full 126-shape
registry: `validate-johnson.ts` (92 OK), `tsc --noEmit` and lint
clean, `verify:face-attach` (3,317,217 checks, 0 failures),
`verify:face-twist` (26,602, 0 failures), and the full Playwright
suite on dicto-node. **All 92 Johnson solids are now in the registry.**
